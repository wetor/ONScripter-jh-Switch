...
void decode_thread_fail(VideoState * videoState)
{
...
int stream_component_open(VideoState * videoState, int stream_index)
{
	...
	case AVMEDIA_TYPE_VIDEO:
	{
		...
		videoState->renderer = SDL_CreateRenderer(screen, -1, SDL_RENDERER_ACCELERATED);
...
void video_display(VideoState * videoState)
{
	...
	if (videoPicture->frame)
	{
		...
		if (++videoState->currentFrameIndex < videoState->maxFramesToDecode)
		{
			...
			SDL_RenderCopy(videoState->renderer, videoState->texture, NULL, &rect);
