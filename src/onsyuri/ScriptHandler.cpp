...
int ScriptHandler::getStringFromInteger( char *buffer, int no, int num_column, bool is_zero_inserted )
{
    ...
    if(coding2utf16->force_utf8) {
        ...
        char format[16];
        if (is_zero_inserted)
            snprintf(format, sizeof(format), "%%0%dd", num_column);
        else
            snprintf(format, sizeof(format), "%%%dd", num_column);
        sprintf(buffer, format, no);
...
int ScriptHandler::findLabel( const char *label )
{
    ...
    char *p = new char[ 256 ];
    snprintf(p, 256, "Label \"%.200s\" is not found.", label);
    errorAndExit( p );
