...
void ONScripter::initSDL()
{
    ...
    if ( SDL_Init( SDL_INIT_VIDEO | SDL_INIT_TIMER  ) < 0 ){
        ...
        if (crashlog) {
            fprintf(crashlog, "SDL_Init failed: %s\n", SDL_GetError());
            fclose(crashlog);
        }
#endif
        exit(-1);
...
#ifdef SWITCH
    // Switch runs fullscreen at 1280x720 in docked, 1280x720 in handheld
    window_flag |= SDL_WINDOW_FULLSCREEN_DESKTOP;
    utils::printInfo("Creating fullscreen window for Switch: %dx%d\n", screen_device_width, screen_device_height);
#endif

    window = SDL_CreateWindow(wm_title_string ? wm_title_string : "ONScripter",
                               window_x, window_y, screen_device_width, screen_device_height, window_flag);
    if (window == NULL) {
        utils::printError("Could not create window: %s\n", SDL_GetError());
#ifdef SWITCH
        FILE *crashlog = fopen("sdmc:/switch/onsyuri/crash.log", "a");
        if (crashlog) {
            fprintf(crashlog, "SDL_CreateWindow failed: %s\n", SDL_GetError());
            fprintf(crashlog, "Requested size: %dx%d, flags: 0x%x\n",
                    screen_device_width, screen_device_height, window_flag);
            fclose(crashlog);
        }
#endif
        exit(-1);
...
#ifdef SWITCH
    utils::printInfo("Creating renderer with flags: 0x%x\n", render_flag);
#endif
    ...
    if (renderer == NULL) {
        ...
        FILE *crashlog = fopen("sdmc:/switch/onsyuri/crash.log", "a");
        if (crashlog) {
            fprintf(crashlog, "SDL_CreateRenderer failed: %s\n", SDL_GetError());
            fprintf(crashlog, "Render flags tried: 0x%x, then 0x0\n", render_flag);
            fclose(crashlog);
        }
#endif
        exit(-1);
    ...
    utils::printInfo("Renderer created: %s\n", renderer_info.name);
#endif
