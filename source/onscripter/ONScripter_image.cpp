                                ...
                                SDL_Surface *src_surface, SDL_Color &color, SDL_Rect *clip, bool rotate_flag)
{
    ...
    if (clip)
    {
        if (AnimationInfo::doClipping(&dst_rect, clip, &clipped_rect))
            return;
    ...
    if (AnimationInfo::doClipping(&dst_rect, &clip_rect, &clipped_rect))
        return;
...
void ONScripter::refreshSurface(SDL_Surface *surface, SDL_Rect *clip_src, int refresh_mode)
{
    ...
    {
        if (nega_mode == 1)
            makeNegaSurface(surface, clip);
        if (monocro_flag)
            makeMonochromeSurface(surface, clip);
        if (nega_mode == 2)
            makeNegaSurface(surface, clip);
    ...
    {
        //MOUSE
        //utils::printInfo("test  image %d x:%d y:%d w:%d h:%d\n",mouse_info.image_surface!=NULL, mouse_info.pos.x,mouse_info.pos.y,mouse_info.pos.w,mouse_info.pos.h);
        //SDL_BlitSurface(cursor_info[0].image_surface, clip_src, surface, &clip);
        //cursor_info[0].pos.x = mouse_info.pos.x;
        //cursor_info[0].pos.y = mouse_info.pos.y;
        //drawTaggedSurface(surface, &cursor_info[0], clip);
