...
void ONScripter::setupAnimationInfo(AnimationInfo *anim, FontInfo *info)
{
    ...
    if (anim->trans_mode == AnimationInfo::TRANS_STRING)
    {
        ...
        { // in case of Sprite, not rclick menu
    ...
    else
    {
            ...
            (!disable_rescale_flag || location == BaseReader::ARCHIVE_TYPE_NONE))
        {
            ...
            int w, h;
            if ((w = src_s->w * screen_ratio1 / screen_ratio2) == 0)
                w = 1;
            if ((h = src_s->h * screen_ratio1 / screen_ratio2) == 0)
                h = 1;
...
void ONScripter::parseTaggedString(AnimationInfo *anim)
{
    ...
    if (buffer[0] == ':')
    {
        ...
        else if (buffer[0] == 'm')
        {
            ...
            char *start = ++buffer;
            while (buffer[0] != ';' && buffer[0] != 0x0a && buffer[0] != '\0')
                buffer++;
            if (buffer[0] == ';')
...
void ONScripter::stopAnimation(int click)
{
    ...
    if (cursor_info[no].image_surface == NULL)
        return;
