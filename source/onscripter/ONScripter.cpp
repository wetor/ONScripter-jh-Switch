...
void ONScripter::calcRenderRect()
{
	...
	{
		vieww = screen_width;
		viewh = (int)(device_height * screen_width / device_width);
	}
	else
	{
		vieww = (int)(device_width * screen_height / device_height);
		viewh = screen_height;
	}
	if (!fullscreen_mode)
	{
		render_view_rect.x = (vieww - screen_width) / 2;
		render_view_rect.y = (viewh - screen_height) / 2;
		render_view_rect.w = screen_width;
		render_view_rect.h = screen_height;
		screen_scale_ratio1 = 1.0;
		screen_scale_ratio2 = 1.0;
	}
	else
	{
		...
		screen_scale_ratio1 = (float)screen_width / vieww;
...
void ONScripter::reset()
{
	...
	if (breakup_cells)
		delete[] breakup_cells;
	if (breakup_mask)
		delete[] breakup_mask;
	if (breakup_cellforms)
		delete[] breakup_cellforms;
...
void ONScripter::flushDirect(SDL_Rect &rect, int refresh_mode)
{
	...
	dst_rect.w += 2;
	dst_rect.h += 2;
	if (AnimationInfo::doClipping(&dst_rect, &screen_rect) || (dst_rect.w == 2 && dst_rect.h == 2))
		return;
#endif
...
void ONScripter::warpMouse(int x, int y)
{
	if (x < 0)
		x = 0;
	else if (x >= screen_device_width)
		x = screen_device_width - 1;
	if (y < 0)
		y = 0;
	else if (y >= screen_device_height)
		y = screen_device_height - 1;
...
executeLabelTop:
	...
	while (current_line < current_label_info.num_of_lines)
	{
		...
		if (kidokuskip_flag && skip_mode & SKIP_NORMAL && kidokumode_flag && !script_h.isKidoku())
			skip_mode &= ~SKIP_NORMAL;
		//TODO: 修改测试，分析解析脚本和执行脚本
		int ret = parseLine();
...
void ONScripter::shadowTextDisplay(SDL_Surface *surface, SDL_Rect &clip)
{
	if (current_font->is_transparent)
	{
		...
		if (AnimationInfo::doClipping(&rect, &clip))
			return;
