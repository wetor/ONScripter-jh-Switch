...
void *FontInfo::openFont(char *font_file, int ratio1, int ratio2)
{
    ...
    if (!fc->next)
    {
        ...
        if (useFile)
            ...
            fc->next->font[0] = TTF_OpenFontRW(SDL_RWFromConstMem(font_cache, font_cache_size), 1, font_size * ratio1 / ratio2);
        ...
        if (useFile)
            ...
            fc->next->font[1] = TTF_OpenFontRW(SDL_RWFromConstMem(font_cache, font_cache_size), 1, font_size * ratio1 / ratio2);
...
SDL_Rect FontInfo::calcUpdatedArea(int start_xy[2], int ratio1, int ratio2)
{
    ...
    else
    {
        ...
        if (rubyon_flag)
            rect.w += font_size_xy[0] - pitch_xy[0];
        if (start_xy[0] == xy[0])
        {
